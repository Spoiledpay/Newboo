...
namespace Boo.Lang.Compiler
{
	...
	{
		private static string SelfKeyword => LanguageAmbiance.SelfKeyword;
		...
		public static CompilerError AmbiguousReference(ReferenceExpression reference, MemberInfo[] members)
		{
		...
		{
			return Instantiate("BCE0018", node, typeName, (whatItIs == null) ? ((object)"not found") : ((object)whatItIs), DidYouMeanOrNull(suggestion));
		}
		...
		{
			return Instantiate("BCE0045", node, "Invalid nested macro context. Check your macro hierarchy");
		}
		...
		{
			return Instantiate("BCE0060", node, signature, incompatibleSignature ? "Method of same name has been found with incompatible signature(s)." : null);
		}
		...
		{
			string text = ((node is Method) ? ((Method)node).Name : ((Property)node).Name);
			return Instantiate("BCE0142", node, parameter, text);
		}
		...
		{
			return Instantiate("BCE0165", dupe.Declaration, dupe.Declaration.Type, previous.Declaration.Type, AstUtil.SafePositionOnlyLexicalInfo(previous.Declaration), LanguageAmbiance.ExceptKeyword);
		}
		...
		public static string ToStringList(IEnumerable names)
		{
			return Builtins.join(from object o in names
				select DisplayStringFor(o) into _
				orderby _
				select _, ", ");
		}
		...
		public static string ToAssemblyQualifiedNameList(List types)
		{
			...
			stringBuilder.Append(((Type)types[0]).AssemblyQualifiedName);
			for (int i = 1; i < types.Count; i++)
			{
				stringBuilder.Append(", ");
				stringBuilder.Append(((Type)types[i]).AssemblyQualifiedName);
			}
		...
		public static string GetSignature(object[] parameters)
		{
			...
			for (int i = 0; i < parameters.Length; i++)
			{
				...
				stringBuilder.Append(parameters[i].GetType());
			}
			stringBuilder.Append(")");
			return stringBuilder.ToString();
		}

		public static string ToNameList(MemberInfo[] members)
		{
			StringBuilder stringBuilder = new StringBuilder();
			for (int i = 0; i < members.Length; i++)
			{
				...
				stringBuilder.Append(members[i].MemberType.ToString());
				stringBuilder.Append(" ");
