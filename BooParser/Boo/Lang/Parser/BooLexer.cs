...
{
	public class BooLexer : CharScanner, TokenStream
	{
		...
		{
			BooExpressionLexer booExpressionLexer = new BooExpressionLexer(getInputState());
			booExpressionLexer.setTabSize(getTabSize());
			booExpressionLexer.setTokenCreator(tokenCreator);
			return booExpressionLexer;
		}
		...
		public BooLexer(TextReader r)
			: this(new CharBuffer(r))
		{
		}

		public BooLexer(InputBuffer ib)
			: this(new LexerSharedInputState(ib))
		{
		}

		public BooLexer(LexerSharedInputState state)
			: base(state)
		{
			initialize();
		}
			...
			else
			{
				...
				else
				{
					...
					case '/':
						...
						if (0 == inputState.guessing)
						{
							num = ((!_preserveComments) ? Token.SKIP : 126);
						}
						break;
					case '=':
						match('=');
		...
		protected void mRE_LITERAL(bool _createToken)
		{
			...
			if (tokenSet_1_.member(cached_LA1))
			{
				mRE_OPTIONS(_createToken: false);
			}
			if (_createToken && token == null && num != Token.SKIP)
			{
				token = makeToken(num);
		...
		protected void mTRIPLE_QUOTED_STRING(bool _createToken)
		{
			...
			while (true)
			{
				...
				if (flag2)
				{
					if (0 == inputState.guessing)
					{
						Enqueue(makeToken(74), text.ToString(length, text.Length - length));
						text.Length = length;
						text.Append("");
					}
		...
		protected void mINTERPOLATED_EXPRESSION(bool _createToken)
		{
			...
			if (cached_LA1 == '$' && cached_LA2 == '{')
			{
				...
				if (0 == inputState.guessing)
				{
					ParseInterpolatedExpression(93, 92);
				}
			}
			else
			{
				...
				if (0 == inputState.guessing)
				{
					ParseInterpolatedExpression(77, 76);
				}
			}
			if (_createToken && token == null && num != Token.SKIP)
			{
			...
			else
			{
				...
				while (true)
				{
					...
					if (flag2)
					{
						if (0 == inputState.guessing)
						{
							Enqueue(makeToken(78), text.ToString(length, text.Length - length));
							text.Length = length;
							text.Append("");
						}
						mINTERPOLATED_EXPRESSION(_createToken: false);
